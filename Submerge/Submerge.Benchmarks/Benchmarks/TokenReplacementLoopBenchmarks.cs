...
public class TokenReplacementLoopBenchmarks
{
    ...
    private TokenReplacementSet[] _tokenReplacementSets;

    // private SubmergeTokenReplacer _submergeTokenReplacer;
    private TokenReplacementEngine _tokenReplacementEngine;
    private FixedTokenMatchSet _fixedTokenMatchSet;
    ...
    public void Setup()
    {
        ...
        for (var i = 0; i < _iterations; i++)
        {
            _tokenReplacementSets[i] = new TokenReplacementSet(new [] {
                _testClasses[i].Name,
                _testClasses[i].Age,
                _testClasses[i].Rank,
        ...
        _tokenReplacementEngine = new TokenReplacementEngine(config);
        _fixedTokenMatchSet = _tokenReplacementEngine.GetFixedTokenMatchSet(_testString.AsMemory());
    }
    ...
    //
    // [Benchmark]
    // public void SubmergeReplaceWithGetMatchesTokenReplacementSetArrayAdd()
    // {
    //     var config = new TokenReplacementConfigurationBuilder().SetTokenStart("{")
    //         .SetTokenEnd("}")
    //         .Build();
    ...
    //     for (var i = 0; i < _iterations; i++)
    //     {
    //         using var tokenReplacementSet = new TokenReplacementSet().AddReplacements(new [] {
    //             _testClasses[i].Name,
    //             _testClasses[i].Age,
    //             _testClasses[i].Rank,
    //             _testClasses[i].Location,
    ...
    public void SubmergeReplaceWithGetMatchesTokenReplacementSetPreCalculated()
    {
        for (var i = 0; i < _iterations; i++)
        {
            var result = _tokenReplacementEngine.Replace(_fixedTokenMatchSet, _tokenReplacementSets[i]);
        }
    }
    
    [Benchmark]
    public void SubmergeReplaceWithGetMatchesInterpolate()
    {
        for (var i = 0; i < _iterations; i++)
        {
            var result = _tokenReplacementEngine.Interpolate(_fixedTokenMatchSet, _tokenReplacementSets[i]);
        }
    }
}